...
uniform sampler3D lut_3d;
uniform sampler1D lut_invgamma[3];
...
vec3 display_gamma_expand(vec3 color) {
    ...
    float r = texture1D(lut_invgamma[0], color.r).r;
    float g = texture1D(lut_invgamma[1], color.g).r;
    float b = texture1D(lut_invgamma[2], color.b).r;
    return vec3(r, g, b);
}
#else
// Assume sRGB
...
void main() {
    ...
    vec3 original = color;
    color = display_gamma_compress(color) * dither_quantization;
#else
